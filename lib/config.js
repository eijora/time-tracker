...
module.exports = function (rl, done) {
    ...
    var configFile = path.join(HOME, '.simple-time-tracker.json');
    var config;
    ...
    if (!config.startDate || !config.dataDir) {
        ...
        rl.question("Original store path: " + config.dataDir + " doesn't exist anymore, Where should we store the data? ", createDir);
    } else {
        done(config);
    }
